	...
	case $tname in
		+([A-Za-z]))
			if [[ -f ./$tname ]];then                       
				...
				while [[ "$num_fields" =~ ['!`@#$%^&*()_+'] ]]  ||  [[ -z "$num_fields" ]] || [[ $num_fields = +([A-Za-z]) ]]; # regix & emptyvalues & string  
				do
					echo -e "you must enter a number \n"
					read -p "Enter The number of Fields: " num_fields  

				done
				# Start loop(name of fields and datatype)
				primary_key="" 
				count=1                                
				...
				do  
					...
					meta_data="field"$seperator"type"$seperator"key"
						...
						myarray[$count]=$field_name$seperator$field_type$seperator$primary_key
